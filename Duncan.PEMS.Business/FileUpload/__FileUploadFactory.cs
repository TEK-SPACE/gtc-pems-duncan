...
namespace Duncan.PEMS.Business.FileUpload
{
    public class FileUploadFactory : BaseFactory
    {
        ...
        /// <param name="customerID"></param>
        /// <returns></returns>
        public List<string> ScheduleJob(string locTypeVal, string[] assetSelected, long fileSelected, DateTime dateScheduled, DateTime dateActivated, string filename, long fileSize, string filehash, int FileType, string ActiveStatus, DateTime FileAdditionDate, int UserID, int customerID)
        {
            ...
            for (var i = 0; i < assetSelected.Length; i++)
            {
                ...
                try
                {
                    ...
                    ////** Find out the Corresponding AreaID from the given AssetID

                    //var items = (from assetname in PemsEntities.Meters
                    //             where assetname.MeterId == assetId && assetname.MeterName != null
                    //             select new FDFilesModel
                    //             {
                    //                 AreaID = assetname.AreaID
                    //             }
                    //                  );
                    ...
                    //{
                    //    //** It indicates that the AreaID or MeterName is null
                    //}

                    //byte areaIDIs = new byte();
                    //if (listitems.Any())
                    //    areaIDIs = (byte)listitems.First().AreaID;
        ...
        public List<FDFilesModel> GetFDFileDetails(DataSourceRequest request, string Roletype, out int total)
        {
            ...
            else
            {
              //  FileExtns = new List<string> { "CCF", "RPG", "PGM", "BIN", "SNSR" };
                FileExtns = new List<string> { "Liberty_SSM_MPB_Config", "Liberty_SSM_CCF", "Liberty_SSM_RPG", "Liberty_SSM_MB_Pgm", "Liberty_SSM_MPB_Pgm", "LIBERTY_SSM_SENSOR_PGM" };
        ...
        public List<SelectListItemWrapper> GetFileTypesForUpload1(string Roletype)
        {
            ...
            {
                FileExtns = new List<string> { "Liberty_SSM_MPB_Config", "Liberty_SSM_CCF", "Liberty_SSM_RPG", "Liberty_SSM_MB_Pgm" };
                List<SelectListItemWrapper> list = (from FDFiles in PemsEntities.FDFileTypes
                                                    where FileExtns.Contains(FDFiles.FileDesc)
