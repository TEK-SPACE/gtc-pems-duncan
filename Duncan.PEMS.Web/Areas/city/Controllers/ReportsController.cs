...
namespace Duncan.PEMS.Web.Areas.city.Controllers
{
    public class ReportsController : PemsController
    {
        ...
        public ActionResult ReportSensor()
        {
            ...
            var occRateObj = new { Name = "Occupancy Report", Description = "This report presents the occupancy status on hourly, weekly and monthly basis.", Action = "OccRate" };
            var sensorProfileObj = new { Name = "Sensor Profile", Description = "This report presents the diagnostics information of sensors.", Action = "SensorProfile" };

            if (model != null)
            {
                if (model.Reports.Count > 0)
                {
                    var occrate = model.Reports.Where(m => m.Action.ToLower() == occRateObj.Action.ToLower()).FirstOrDefault();
                    if (occrate != default(ReportViewModel))
                    {
                        occrate.Name = occRateObj.Name;
                        occrate.Description = occRateObj.Description;
                    }

                    var SensorProfile = model.Reports.Where(m => m.Action.ToLower() == sensorProfileObj.Action.ToLower()).FirstOrDefault();
                    if (SensorProfile != default(ReportViewModel))
                    {
                        SensorProfile.Name = sensorProfileObj.Name;
                        SensorProfile.Description = sensorProfileObj.Description;
                    }
                }
            }
