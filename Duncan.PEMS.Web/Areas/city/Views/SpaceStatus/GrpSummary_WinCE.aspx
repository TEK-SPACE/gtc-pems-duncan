...
.siw {width:<%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("siw", true, this.Model) %>; min-width:<%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("siw", true, this.Model) %>;
     max-width:<%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("siw", true, this.Model) %>;}
...
<script type="text/javascript">
   ...
   function drilldownToSelected() {
    ...
    function refreshCurrentInfo() {
     ...
     jx.load('<%= Url.Action("GrpSummaryPartial","SpaceStatus") %>' + 
          '?G=' + encodeURIComponent('<%= ViewData["groupType"]%>') + 
          '&V=' + encodeURIComponent('<%= ViewData["viewType"]%>') + 
          '&CID=' + encodeURIComponent(<%= (this.ViewData["CustomerCfg"] as CustomerConfig).CustomerId.ToString() %>),
          function(data){
...
--%>
  ...
  <%-- 
  <div  class="side-by-side2 clearfix2" style="border-style:none; overflow:visible; display:table;" >
  --%>
  <%-- 
  <span class="desktopBackBtn" style="display:inline-block; height:24px; line-height:24px; vertical-align:middle;">
  <input type="image" value="&nbsp;" src="<%= Url.Content("~/content/images/arrow-left-blue.png") %>"
  style="width:40px; height:24px; border-style:none; background-image: url(<%= Url.Content("~/content/images/arrow-left-blue.gif") %>);
