...
<script type="text/javascript">
    ...
    function drilldownToSelected() {
        window.location.replace('<%= Url.Action("SpaceDetail","SpaceStatus") %>' + '?targetID=' + encodeURIComponent(targetID) + '&CID=' + encodeURIComponent(customerID));
    }
</script>
...
<div id="myheader" style="position:absolute; top:0px; left:0px; overflow:hidden; color:#004475;">
  <div style="font-style:italic; width:204px; text-align:center; font-size: 14px; color:Blue; padding-bottom:4px;">Vacancies - Meter <%=ViewData["CurrentMeter"]%></div>
 ...
 <%= Html.DropDownList("GroupSelect", ViewData["MeterList"] as SelectList, new { @class = "chzn-select", style = "min-width:100px;" })%>
  ...
  <script type="text/javascript">
      $("#GroupSelect").chosen().change(function () {
          //   alert("this is an alert. Selected value = " + $("#GroupSelect option:selected").val());
       
          //window.location.replace('#' + $("#GroupSelect option:selected").val());
...
<%= MobileOccupancyStatusHelpers.ListView_GroupBySpace(this.Model, (this.ViewData["CustomerCfg"] as CustomerConfig))%>
