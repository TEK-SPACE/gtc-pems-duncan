...
.sectionWidth2 {width:<%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("sectionWidth2", true, this.Model) %>;}
...
.siw2 {width:<%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("siw2", true, this.Model) %>;}
...
@media screen and (max-width: <%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("mediaWidth_VerySmall", true, this.Model) %>) <%-- /* for really small mobile devices */ --%>
{
  ...
  .statusIcon{/*display:none;*/<%= SpaceStatusHelpers.StatusIconDisplayForDisplayStyle(true, this.ViewData["displayStyle"].ToString()) %> }
  
  .desktopBackBtn{display:none;} 
  .desktopRefreshBtn{display:none;}
  .desktopGearsBtn{display:none;}
...
.sectionWidth2 {width:<%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("sectionWidth2", false, this.Model) %>;}
...
.siw2 {width:<%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("siw2", false, this.Model) %>;}
}

@media screen and (min-width: 400px) <%-- /* for displays wide enough for 1 column and the normal navigation bar too */ --%>
{
  .desktopBackBtn{display:block;} 
...
@media screen and (min-width: <%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("mediaWidth_4Column", false, this.Model) %>) <%-- /* for displays wide enough for 4 columns */ --%>
{
  ...
  .headerarea {display:block; height:50px; overflow:visible;}
  .footerarea  { height:28px; left:0px; overflow:hidden;  border-style:none; }
  .contentarea{ max-width: /*1332px*/ <%= SpaceStatusHelpers.GroupSummary_CSSDynamicValue("mediaWidth_4Column_MaxWidth", false, this.Model) %>;  /*max-height:522px;*/ overflow:auto; background-color:rgb(239, 243, 251);}
...
<script type="text/javascript">
    ...
    function refreshCurrentInfo() {
        ...
        */
        ...
        $('#loading').css('display', 'inline-block');
        $('#partial1').load('<%= Url.Action("SpcSummaryPartial","SpaceStatus") %>' + 
...
<div id="myheader" class="headerarea">
  ...
  <div style="font-style:italic; width:204px; overflow:hidden; max-height:15px; text-align:center; font-size: 14px; color:Blue; padding-bottom:4px;">Space - Summary
  </div>
 ...
 <%= Html.DropDownList("GroupSelect", ViewData["groupChoices"] as SelectList, new { @class = "chzn-select", style = "min-width:100px;" })%>
  ...
  <div class="desktopGearsBtn" >
   <input type="button" value="&nbsp;" style="width:40px; height:24px; border-style:none; background-image: url(<%= Url.Content("~/content/images/Gears3Blue.gif") %>);
