...
<title>Violations - Space Detail</title>


<% if ((ViewData["IsWinCE"] != null) && (Convert.ToBoolean(ViewData["IsWinCE"]) == true)) { %>    
 <style type="text/css">
   body {font-size: 12px; line-height :12px; height:100%; width:224px; max-width:240px; background-color:White;} 
   .contentarea {border-style:none; background-color:White; width:266px;}
 </style>
<% } else { %>
 <style type="text/css">
   body {font-size: 12px; line-height :12px; height:100%; background-color:White;} 
   .contentarea {border-style:none; background-color:White;}
 </style>
<% }  %>            
...
.dtcHead {font-size :12px; line-height:12px; display:block; margin:0px; float:left; white-space:nowrap; background-color:rgb(239, 243, 251); border-left:2px solid rgb(239, 243, 251); border-top:1px solid rgb(239, 243, 251); border-bottom:1px solid rgb(239, 243, 251); border-right:none; }

.dtc_Detail {width:218px; font-size :12px; line-height:12px; height: 12px; display:block; margin:0px; margin-left:2px; float:left; white-space:nowrap;}
.dtcLeft_Detail {width:109px; font-size:12px;  line-height:12px; display:block;  margin:0px; margin-left:2px; float:left; white-space:nowrap;  }
.dtcRight_Detail {width:109px; font-size:12px; line-height:12px; display:block;  margin:0px; float:left; white-space:nowrap;  }
...
.whiteCell_Detail{background-color:white; color:Black;}
.NavyTextCell_Detail{background-color:white; color:#000066;}
.appleblueCell_Detail{background-color:#1A2644; color:White;}
.orangeCell_Detail{background-color:Orange; color:White;}
.greenCell_Detail{background-color:Green; color: White;}
.redCell_Detail{background-color:Red; color: White;}
.grayCell_Detail{background-color:#484848; color: White;}
.purpleCell_Detail{background-color:#484848; color: White;}
.ltgreenCell_Detail{background-color:#9BBB59; color: White;}
...
{
body {font-size: 12px; line-height :12px; height:100%; width:100%; max-width:100%;} 
.dtNoLinesOrGaps {display:inline-block; margin:0px; padding:0px; border:none; border-collapse: collapse; border-spacing: 0px; border-width:0px;}
.contentarea {border-style:none; background-color:White; width:100%; max-width:1024px;}
}
...
<script type="text/javascript">
    ...
    function navigateBackClick(){
      ...
      */
      ...
      window.location.href = '<%= Url.Action("StayVioSpcSum","SpaceStatus") %>' + 
    ...
    function refreshCurrentInfo() {
     ...
     jx.load('<%= Url.Action("StayVioSpcDetailPartial","SpaceStatus") %>' + 
        '?T=' + encodeURIComponent('<%= Convert.ToString(this.ViewData["originalTarget"]) %>') + 
        '&PT=' + encodeURIComponent('<%= Convert.ToString(this.ViewData["parentTarget"]) %>') +
        '&G=' + encodeURIComponent('<%= Convert.ToString(ViewData["groupType"]) %>') + 
        '&V=' + encodeURIComponent('<%= Convert.ToString(ViewData["viewType"]) %>') + 
        '&CID=' + encodeURIComponent('<%= (this.ViewData["CustomerCfg"] as CustomerConfig).CustomerId.ToString() %>') +
...
<div style="width:224px; margin:0px; padding:0px; border:none; border-collapse: collapse; border-spacing: 0px; border-width:0px;">
  ...
  <div style="font-style:italic; width:222px; overflow:hidden; max-height:15px; text-align:center; font-size: 14px; color:Blue; padding-bottom:4px;">Violations - Space Detail</div>
...
<span id="partial1">
  <% Html.RenderPartial("pv_StayVioSpcDetailContent"); %>
</span>
