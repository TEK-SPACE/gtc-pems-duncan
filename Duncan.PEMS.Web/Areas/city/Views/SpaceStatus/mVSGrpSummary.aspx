...
<%-- Supposedly better settings for Kindle Fire --%>
...
<title>Mobile VS Group Summary</title>
...
<%-- /* This media query came from: http://www.reigndesign.com/blog/tips-tricks-for-how-to-not-have-your-new-site-look-awful-on-phones/ */ --%>
/* kindle fire ----------- */
...
<script type="text/javascript">
    ...
    function drilldownToSelected() {
        ...
        if (targetID != "") {
           stopStatusRefreshTimer();
            ...
            window.location.href = '<%= Url.Action("mVSSpcSummary","SpaceStatus") %>' + '?T=' + encodeURIComponent(targetID) + 
    ...
    function refreshCurrentInfo() {
        ...
        */
        ...
        $('#loading').css('display', 'inline-block');
        $('#partial1').load('<%= Url.Action("mVSGrpSummaryPartial","SpaceStatus") %>' + 
...
<div id="myheader" class="headerarea">
  ...
  <div style="font-style:italic; width:204px; overflow:hidden; max-height:15px; text-align:center; font-size: 14px; color:Blue; padding-bottom:4px;">Vacancies - Summary</div>
...
<span id="partial1">
  <% Html.RenderPartial("pv_mVSGrpSummaryContent"); %>
</span>

<%-- <%= MobileOccupancyStatusHelpers.VSSummaryByGroups(this.Model, this.ViewData)%> --%>
