...
.loading {display: inline-block; left:8px; height: 40px; line-height: 40px; top:58px; width:188px; border:1px solid #000; position:fixed; background-color:#FFFF00; background-color: rgba(255,255,0,.8); 
...
<script type="text/javascript">
    ...
    function drilldownToSelected() {
        if (targetID == ""){
          ...
          targetID = '<%= this.ViewData["originalTarget"].ToString() %>';
        }
        ...
        if (targetID != "") {
            ...
            window.location.href = '<%= Url.Action("mVSSpcDetail","SpaceStatus") %>' + 
    ...
    function refreshCurrentInfo() {
        ...
        */
        ...
        $('#partial1').load('<%= Url.Action("mVSSpcDetailPartial","SpaceStatus") %>' + 
          '?T=' + encodeURIComponent('<%= this.ViewData["originalTarget"].ToString() %>') + 
          '&PT=' + encodeURIComponent('<%= this.ViewData["parentTarget"].ToString() %>') +
...
<div id="myheader" class="headerarea">
  <div style="font-style:italic; width:204px; overflow:hidden; max-height:15px; text-align:center; font-size: 14px; color:Blue; padding-bottom:4px;">Space Sensor Details</div>
  ...
  <div  class="side-by-side clearfix" style="height:400px; border-style:none; overflow:visible;" >
   ...
   <div>
    ...
    <%-- This is at a space-detail level, so there are no other spaces that can be scrolled to on this page --%>
    <%-- 
    <script type="text/javascript">
...
<span id="partial1">
  <% Html.RenderPartial("pv_mVSSpcDetailContent"); %>
</span>

  <%-- <%= MobileOccupancyStatusHelpers.VSSpaceDetails(this.Model, this.ViewData)%> --%>
