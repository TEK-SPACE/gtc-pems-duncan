...
$.fn.dataTableExt.oApi.fnColReorder = function ( oSettings, iFrom, iTo )
{
	 ...
	 */
	if (oSettings.aoColumns[iFrom]!= null && oSettings.aoColumns[iFrom].bVisible )
	{
...
ColReorder.prototype = {
	...
	"_fnConstruct": function ()
	{
		...
		if (typeof this.s.init.fnInsertCallback != 'undefined') {
			this.s.insertCallback = this.s.init.fnInsertCallback;
		}
		
		if (typeof this.s.init.fnRemoveCallback != 'undefined') {
			this.s.removeCallback = this.s.init.fnRemoveCallback;
		}
	...
	"_fnMouseDownHiddenHelper": function (e, nTh) {
		var that = this,
			aoColumns = this.s.dt.aoColumns;
		...
		this.s.mouse.offsetY = e.pageY - offset.top;
		this.s.mouse.target = nTh;
		this.s.mouse.targetIndex = this.s.aoTargets.length - 1;
		this.s.mouse.fromIndex = this.s.aoTargets.length - 1;
		...
		/* Add event handlers to the document */
		$(document).bind('mousemove.ColReorder', function (e) {
			that._fnMouseMoveHiddenHelper.call(that, e);
		});

		$(document).bind('mouseup.ColReorder', function (e) {
			that._fnMouseUpHiddenHelper.call(that, e);
		});
	},
	...
	"_previousWasInRemove" : false,

	"_fnMouseMove": function ( e )
	{
		var that = this;
		var isRemove = this._fnIsRemove(e);
		if ( this.dom.drag === null )
		{
			...
			if (!isRemove) {
				this._fnCreateDragNode(e);
			} else {
				this._fnCreateDrag(e);
			}
		}
		
		/* Position the element - we respect where in the element the click occured */
		this.dom.drag.style.display = '';
		this.dom.drag.style.left = (e.pageX - this.s.mouse.offsetX) + "px";
		this.dom.drag.style.top = (e.pageY - this.s.mouse.offsetY) + "px";

		if (isRemove) {
			this._fnRemovePointerNode(e);
			$(this.dom.drag).css('border', '2px solid red');
			this._previousWasInRemove = true;
		} else {
			if (this._previousWasInRemove) {
				this._fnCreatePointer(e);
			}
			this._previousWasInRemove = false;
			$(this.dom.drag).css('border', '');
		}
		...
		for ( var i=1, iLen=this.s.aoTargets.length ; i<iLen ; i++ )
		{
			...
			{
				if (this.dom.pointer != null) {
					this.dom.pointer.style.left = this.s.aoTargets[i - 1].x + "px";
					this.dom.pointerIcon.style.left = this.dom.pointer.style.left;
				}
	...
	"_fnMouseMoveHiddenHelper": function (e) {
		var that = this;
		...
		/* Position the element - we respect where in the element the click occured */
		this.dom.drag.style.display = 'none';
		this.dom.drag.style.left = (e.pageX - this.s.mouse.offsetX) + "px";
		this.dom.drag.style.top = (e.pageY - this.s.mouse.offsetY) + "px";
	...
	"_fnRemovePointerNode": function (e) {
		if (this.dom.pointer !== null) {
			document.body.removeChild(this.dom.pointer);
			document.body.removeChild(this.dom.pointerIcon);
			this.dom.pointer = null;
			this.dom.pointerIcon = null;
		}
	},
	...
	"_fnClearDrag": function(e) {
		$(document).unbind( 'mousemove.ColReorder' );
		$(document).unbind( 'mouseup.ColReorder' );
		this._fnRemoveDragNode(e);
		this._fnRemovePointerNode(e);
	},
	
	"_fnIsRemove" : function(e) {
		var isRemove = false;
		var px = e.pageX,
			py = e.pageY;
		var table$ = $(this.s.dt.nTable);
		var tOffset = table$.offset();
		if (px < tOffset.left - 100 || px > tOffset.left + table$.width() + 100 ||
			py < tOffset.top || py > tOffset.top + table$.height()) {
			isRemove = true;
		}
		return isRemove;
	},
	...
	"_fnMouseUp": function ( e )
	{
		...
		if ( this.dom.drag !== null )
		{
			...
			if (!isRemove) {
				...
				if (this.s.removeCallback !== null) {
					this.s.removeCallback.call(this, e, this.s.mouse.fromIndex);
				}
			}
	...
	"_fnMouseUpHiddenHelper": function (e) {
		var that = this;
		...
		if (this.dom.drag !== null) {
			...
			if (this.s.insertCallback !== null) {
				this.s.insertCallback.call(this, e, this.s.mouse.toIndex);
			}
	...
	"_fnCreatePointer" : function(e) {
		...
		this.dom.pointerIcon = document.createElement('div');
		this.dom.pointerIcon.className = "DTCR_pointer_Icon";
		this.dom.pointerIcon.style.position = "absolute";

		if (this.s.dt.oScroll.sX === "" && this.s.dt.oScroll.sY === "") {
			this.dom.pointer.style.top = $(this.s.dt.nTable).offset().top + "px";
			this.dom.pointerIcon.style.top = this.dom.pointer.style.top;
			this.dom.pointer.style.height = $(this.s.dt.nTable).height() + "px";
			this.dom.pointerIcon.style.height = this.dom.pointer.style.height;
		}
		else {
		...
		document.body.appendChild(this.dom.pointerIcon);
	},
	
	"_fnCreateDrag" : function(e) {
		var that = this;
