...
(function($){ 	
	...
	h.append("<style type='text/css'>  .JColResizer td, .JColResizer th{overflow:hidden;padding-left:0!important; padding-right:0!important;}  .JCLRgrips{ height:0px; position:relative;} .JCLRgrip{margin-left:-5px; position:absolute; z-index:5; } .JCLRgrip .JColResizer{position:absolute;background-color:red;filter:alpha(opacity=1);opacity:0;width:10px;height:100%;top:0px} .JCLRLastGrip{position:absolute; width:1px; } .JCLRgripDrag{ border-left:1px dotted black;	}</style>");
	...
	var createGrips = function(t){
		...
		th.each(function(i){						//iterate through the table column headers			
			...
			if (i < t.ln)	g.mousedown(onGripMouseDown).append(t.opt.gripInnerHtml).append('<div class="'+SIGNATURE+'" style="cursor:'+t.opt.hoverCursor+'"></div>'); //bind the mousedown event to start dragging 
	...
	var recalculateTableWidth = function (t) {
		var sum = 0;
	...
	*/
	var syncCols = function(t,i,isOver) {
		...
		// new
		c.width(w + PX);
		c.css('min-width', w + PX);
		c.css('max-width', w + PX);
		t.cg.eq(i).width(w + PX);
		if (isOver) {
			c.w = w;
		}
		recalculateTableWidth(t);
		// old
		/*
