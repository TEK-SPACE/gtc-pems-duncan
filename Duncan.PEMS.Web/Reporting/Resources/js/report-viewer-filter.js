function GetRenderedReportSet(invalidateInCache, additionalParams) {
	console.log('custom getRenderedReportSet called, ap=' + additionalParams);
...
function GotRenderedReportSet(returnObj, id) {
	...
	var pageLinkNum = 0;
	var pageLink = document.getElementById('paginationControl' + pageLinkNum);
	while (pageLink != null) {
		var oldHref = pageLink.getAttribute('href');
		if (oldHref != null && oldHref != '' && oldHref.indexOf('javascript:') == 0) {
			var pureRef = oldHref.substr(11);
			console.log(pureRef);
			pageLink.setAttribute('href', '#');
			pageLink.setAttribute('onclick', 'reportViewerFilter.refreshFilters(false, function(){' + pureRef + ';});');
		}
		pageLinkNum++;
		pageLink = document.getElementById('paginationControl' + pageLinkNum);
	}
}

function ParseDateTime(dts, fmt) {
	console.log('Date parse call for: ' + dts + ' with format ' + fmt);
	var fmtLow = fmt.toLowerCase();
	var nodes = dts.split(' ');
	var date = nodes[0];
	var time = nodes[1];
	var dateNodes = date.split('/');
	var year = dateNodes[2];
	var month;
	var day;
	if (fmtLow == 'dd/mm/yy') {
		month = dateNodes[1];
	...
	else if (fmtLow == 'mm/dd/yy') {
		month = dateNodes[0];
		day = dateNodes[1];
	}
	else {
		alert('Unexpected DateTime format');
		return;
	}
	var timeNodes = time.split(':');
	var hour = timeNodes[0];
	var minute = timeNodes[1];
	var parsedDate = new Date(year, month - 1, day, hour, minute, 0, 0);
...
function ReportViewerFilters() {
	...
	var _stringToDate = function (dateStr, format, targetFormat) {
		if (dateStr == null)
			return null;
		if (dateStr == '')
			return null;
		...
		try {
			...
			var d0 = d.split('/');
			var t0 = t.split(':');
			var year = parseInt(d0[2]);
			var month = parseInt(d0[format == 'MMDDYYYY' ? 0 : 1]);
			var day = parseInt(d0[format == 'MMDDYYYY' ? 1 : 0]);
			var hour = parseInt(t0[0]);
			var minute = parseInt(t0[1]);
	...
	var _commitFiltersData = function (updateReportSet, callback) {
		...
		for (var index = 0; index < _filtersData.RegularFilters.length; index++) {
			...
			if (_filtersData.RegularFilters[index].ControlType == 5 || _filtersData.RegularFilters[index].ControlType == 9) {
				var dateFormat = reportViewerFilterSettings.dateTimeFormat;
				var dateTargetFormat = reportViewerFilterSettings.dateTimeServerFormat;
				var f = _filtersData.RegularFilters[index];
				if (filterObj.Values != null && filterObj.Values.length == 2) {
					filterObj.Values[0] = _stringToDate(filterObj.Values[0], dateFormat, dateTargetFormat);
					filterObj.Values[1] = _stringToDate(filterObj.Values[1], dateFormat, dateTargetFormat);
				}
				if (filterObj.Value) {
					filterObj.Value = _stringToDate(filterObj.Value, dateFormat, dateTargetFormat);
				}
			}
		...
		if (affectingFilters.length > 0) {
			$.each(affectingFilters, function (iRf, rf) {
				...
				if (rf.ControlType == 5 || rf.ControlType == 9) {
					var dateFormat = reportViewerFilterSettings.dateTimeFormat;
					var dateTargetFormat = reportViewerFilterSettings.dateTimeServerFormat;
					fvl = _stringToDate(fData.value1, dateFormat, dateTargetFormat);
					fvr = _stringToDate(fData.value2, dateFormat, dateTargetFormat);
				}
	...
	var _generateFilterControlHtml = function (filter) {
		...
		if ([5, 9].indexOf(filter.ControlType) >= 0) {
			$.each(result$, function (iRes, res) {
				...
				var dateFormat = reportViewerFilterSettings.dateTimeFormat == 'DDMMYYYY' ? 'dd/mm/yy' : 'mm/dd/yy';
				...
				datePickerTarget$.datetimepicker({
					...
					onSelect: function (date, datepicker$) {
						console.log('parsing this date');
						var thisDate = ParseDateTime(datepicker$.input.val(), dateFormat);
						var currInput$ = $(this);
						...
						if (nextInput$.length > 0) {
							if (nextInput$.val() != null && nextInput$.val() != '') {
								console.log('parsing next date');
								var nextDate = ParseDateTime(nextInput$.val(), dateFormat);
							...
							if (prevInput$.val() != null && prevInput$.val() != '') {
								console.log('parsing prev date');
								var prevDate = ParseDateTime(prevInput$.val(), dateFormat);
		...
		if (filter.Values != null && filter.Values.length > 0 && filter.ControlType != 1) {
			filterTemplateContent$.find('input').each(function (iInput, input) {
				...
				if (filter.FromServer && filter.ControlType == 5) {
					var dateFormat = _filterSettings.dateTimeFormat;
					var dateTargetFormat = _filterSettings.dateTimeServerFormat;
					$input.val(_stringToDate(filter.Values[iInput], dateTargetFormat, dateFormat));
				}
	...
	var _initializeReportTitle = function () {
	    ...
	    pageTitle$.empty();
	    if (reportCategory) {
	        pageTitle$.append($(reportCategory + ' / <span>' + reportName + '</span>'));
	    } else {
	        pageTitle$.append($('<span>' + reportName + '</span>'));
	    }
	};
	...
	var _initializeControl = function () {
		$.ajax({
            url: '/Reporting/Resources/Html/report-viewer-filter-templates.html'
		}).done(function (data) {
			$(data).appendTo('head');
			_renderFilters();
